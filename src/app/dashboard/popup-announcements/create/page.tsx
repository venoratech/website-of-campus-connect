...
export default function PopupAnnouncementForm() {
  ...
  // New state for multiple college selection
  ...
  // If editing, fetch the popup announcement data and college associations
  useEffect(() => {
      ...
      const fetchPopupAnnouncement = async () => {
        try {
          ...
          if (data) {
            ...
            // If it's global, we don't need to fetch college associations
            if (!data.is_global) {
              // Fetch the associated colleges from popup_announcement_colleges table
              ...
              if (!collegeError && collegeData) {
                // Extract college IDs
                const collegeIds = collegeData.map(item => item.college_id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const popupData = {
        ...
        is_global: isGlobal,
        college_id: null, // No longer using single college_id
        target_audience: ['all']
      };
      ...
      if (isEditing) {
        ...
        // Handle college associations if not global
        if (!isGlobal) {
        ...
        if (!isGlobal && newPopup && newPopup.length > 0) {
          const popupId = newPopup[0].id;
          
          // Insert college associations
          if (selectedColleges.length > 0) {
            const collegeAssociations = selectedColleges.map(collegeId => ({
              popup_id: popupId,
              college_id: collegeId
  ...
  // Preview component
  const PopupPreview = () => {
    // Get college names for the preview
